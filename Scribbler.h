...
public:
    ...
    */
    ...
    enum class color { RED = BACKGROUND_RED, GREEN = BACKGROUND_GREEN, BLUE = BACKGROUND_BLUE, WHITE_TEXT = 0x0F, WHITE_BACKGROUND_BLACK_TEXT = 0xF0, BLACK = 0x00 };

    // using the color attribute of characters in console, draw rectangle
    void drawRectangle_color(
        uint32_t x, 
        uint32_t y, 
        uint32_t width, 
        uint32_t height, 
        ...
        if (x < (uint32_t)_coordBufSize.X && y < (uint32_t)_coordBufSize.Y) {
            ...
            for (uint32_t i = 0; i < height && y + i < (uint32_t)_coordBufSize.Y; i++) {
                for (uint32_t j = 0; j < width && x + j < (uint32_t)_coordBufSize.X; j++) {

                    _chiBuffer[(x + _coordBufSize.X * y) + (j + (i)*_coordBufSize.X)].Attributes = (WORD)c;
                }
            }
        }
    }
    ...
    void drawToConsole() {
        ...
        _fSuccess = WriteConsoleOutput(
            _hScreenBuffer,    // screen buffer to write to
    ...
    Scribbler() {
        ...
        SetConsoleTitle(L"Fake 3D");

        // get console mode
        if (GetConsoleMode(_windowHandle, &_consoleMode_data)) exit(1);

        // set console mode. Hinders character input
        SetConsoleMode(_windowHandle, (ENABLE_EXTENDED_FLAGS | _consoleMode_data & ~ENABLE_MOUSE_INPUT & ~ENABLE_INSERT_MODE & ~ENABLE_LINE_INPUT));

        //stores the console's current dimensions
        GetWindowRect(_windowHandle, &_r); 

        // changes the console's dimensions. dimensions perfectly fit all charcacters
        MoveWindow(_windowHandle, _r.left, _r.top, SCREEN_WIDTH_PIXELS, SCREEN_HEIGHT_PIXELS, TRUE); 

        // disables the window's resizable property
        SetWindowLong(_windowHandle, GWL_STYLE, GetWindowLong(_windowHandle, GWL_STYLE) & ~WS_MAXIMIZEBOX & ~WS_SIZEBOX);
        ...
        // loop through character_info array buffer / screen buffer and set every character to '/' or '\' with black background and white text
        for (CHAR_INFO& c : _chiBuffer) {

            if (a) { c.Char.UnicodeChar = L'/'; a = false; }
            else { c.Char.UnicodeChar = L'\\'; a = true; }
