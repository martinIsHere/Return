...
float evaluateDistanceBetweenRayAndWall(
    const LineSegment& v,  // Casted Ray
    ...
    float r = float(V_y * D_x + V_x * C_y - V_y * C_x - V_x * D_y) / float(V_x * W_y - V_y * W_x);
    float t = float(W_y * C_x + W_x * D_y - W_y * D_x - W_x * C_y) / float(W_x * V_y - W_y * V_x);
...
void updateDistances(
    ...
    for (int i = 0; i < amountOfArrays; i++) {
        ...
        for (int j = 0; j < amountOfActiveWalls; j++) {
            ...
            // calculate distance between buffer with current ray-vector and current active wall
            evaluatedDistance = evaluateDistanceBetweenRayAndWall(rayFromPointBuf, activeWalls[j]);
            ...
            if (((evaluatedDistance < distances[i]) && evaluatedDistance > 0) || distances[i] == -1.0f) distances[i] = evaluatedDistance;
        }
    }
}
...
void draw3DRenderColumns(
    ...
    const float* const distances, // distances from playerPos along vectors in Rays
    ...
    const uint32_t remainderColWidth = colWidth + (uint32_t)(SCREEN_WIDTH % amountOfRays);
    ...
    if (distances[0] >= 1.0f) {
        // set offset value
        offsetY = screenMidPointY - uint32_t(float(SCREEN_HEIGHT) / distances[0] * 0.5f);

        // draw column based on distances
        s->drawRectangle_color(0, offsetY, remainderColWidth, uint32_t(SCREEN_HEIGHT / distances[0]), Scribbler::color::WHITE_BACKGROUND_BLACK_TEXT);
    }
    // loop through rays
    for (int i = 1; i < amountOfRays; i++) {
        ...
        if (distances[i] >= 1.0f) {
            ...
            // set offset value
            offsetY = screenMidPointY - uint32_t(float(SCREEN_HEIGHT) / distances[i] * 0.5f);

            // draw column based on distances
            s->drawRectangle_color(remainderColWidth + (i - 1) * colWidth, offsetY, remainderColWidth, uint32_t(SCREEN_HEIGHT / distances[i]), Scribbler::color::WHITE_BACKGROUND_BLACK_TEXT);
        }
    }
}
